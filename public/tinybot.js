  ...
  async function predictionLoop() {
    ...
    if (state.model && video.readyState >= 2) {
      try {
        ...
        const recognized = results.map((item) => ({ move: normalizeClass(item.className), confidence: item.probability })).filter((item) => item.move).sort((a, b) => b.confidence - a.confidence)[0];
        if (recognized) { state.latest = recognized; if (!state.playing) renderPrediction(recognized.move, recognized.confidence); }
