...
function ReactionLab(){const [index,setIndex]=useState(0);const [mixed,setMixed]=useState(false);const r=reactions[index];return <div className="experience two-column"><div className="reaction-bench"><div className={`flask ${mixed?r.effect:""}`}><i/><span>{mixed?"PRODUCTS":"REACTANTS"}</span></div><select value={index} onChange={e=>{setIndex(+e.target.value);setMixed(false)}}>{reactions.map((r,i)=><option value={i} key={r.name}>{r.name}</option>)}</select><button onClick={()=>setMixed(true)}>COMBINE SYMBOLICALLY</button></div><LabIntro objective="Classify curriculum reactions and interpret balanced equations." ai="Gestures may select reactant cards; reaction selection and balancing are rule-based." limit="This is a safe symbolic lab. It gives no quantities or real mixing instructions."><div className="equation-card"><small>BALANCED EQUATION</small><h3>{r.eq}</h3><b>{r.name} reaction</b><p>{mixed?r.result:"Combine the virtual reactants to observe the result."}</p></div></LabIntro></div>}
...
function CarbonLab(){const [m,setM]=useState(molecules[4]);return <div className="experience two-column"><div className="molecule-stage"><div className={`molecule ${m.bond}`}>{Array.from({length:m.c},(_,i)=><span className="atom-c" key={`c${i}`}>C</span>)}{Array.from({length:m.o},(_,i)=><span className="atom-o" key={`o${i}`}>O</span>)}{Array.from({length:Math.min(m.h,8)},(_,i)=><i key={`h${i}`}>H</i>)}</div><div className="molecule-picker">{molecules.map(x=><button className={m.name===x.name?"active":""} onClick={()=>setM(x)} key={x.name}>{x.name}</button>)}</div></div><LabIntro objective="Relate carbon valency, bond type, formula and functional groups." ai="Hand drag can place atoms; a valency rule engine checks legal bonding." limit="Flat structural diagrams simplify real three-dimensional molecular shapes."><div className="element-card"><small>VALID MOLECULE</small><h3>{m.name}</h3><span>Molecular formula <b>{m.formula}</b></span><span>C–C bond <b>{m.bond}</b></span><span>Series / group <b>{m.group}</b></span><span>Carbon valency <b>4</b></span></div><KnowledgeCheck questions={["Build ethanol","Build ethene","Identify the functional group"]}/></LabIntro></div>}
